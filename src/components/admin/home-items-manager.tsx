...
import { Plus, Pencil, Trash2, Upload, ImageIcon, ExternalLink } from "lucide-react";
...
export function HomeItemsManager({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "space-y-4" : "space-y-6"}>
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className={embedded ? "text-xl font-bold" : "text-2xl font-bold"}>Store 1 home page items</h2>
          <p className="text-sm text-muted-foreground">
            Yahan se har card ki price, name, picture, sold count, hot label aur hide/show control kar sakte hain.
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editing ? "Edit Store 1 item" : "New Store 1 item"}</DialogTitle>
            </DialogHeader>
            <form onSubmit={save} className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 space-y-2">
                <Label>Custom picture / logo</Label>
                <div className="flex items-center gap-3">
                  {form.logo_url ? (
                    <img src={form.logo_url} alt="" className="h-16 w-16 rounded-xl border object-cover" />
                  ) : (
                    <div className="grid h-16 w-16 place-items-center rounded-xl border bg-secondary">
              ...
              <div className="flex items-end gap-4">
                <label className="flex items-center gap-2 text-sm">
                  <Switch checked={form.hot} onCheckedChange={(v) => setForm({ ...form, hot: v })} /> 🔥 Hot
                </label>
                <label className="flex items-center gap-2 text-sm">
              ...
              <tr key={p.id} className="border-t">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium">
                        {p.name} {p.hot && <span className="text-red-500">🔥</span>}
                      </div>
                ...
                <td className="p-3">
                  <div className="flex items-center justify-end gap-1">
                    {p.logo_url && (
                      <a href={p.logo_url} target="_blank" rel="noreferrer">
